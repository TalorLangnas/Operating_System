...
int main(int argc, char *argv[]) {
    ...
    // Allocate memory for the encodeing buffer
    char *encoding_word = (char *)malloc((count + 1) * sizeof(char));
    if (encoding_word == NULL) {
        printf("Error allocating memory\n");
        fclose(src_file);
        fclose(dst_file);
        return 1;
...
// Encode the input word:
    ...
    int len = encode(input_word, encoding_word, count * sizeof(char), codec);
    if (len < 0) {
        printf("Failed to encode the input word\n");
        return 1;
    }
